...
strophe_js.Strophe.addConnectionPlugin("muc", {
  ...
  init: function(conn) {
    ...
    strophe_js.Strophe.addNamespace(
      "MUC_OWNER",
      strophe_js.Strophe.NS.MUC + "#owner"
    );
    strophe_js.Strophe.addNamespace(
      "MUC_ADMIN",
      strophe_js.Strophe.NS.MUC + "#admin"
    );
    strophe_js.Strophe.addNamespace(
      "MUC_USER",
      strophe_js.Strophe.NS.MUC + "#user"
    );
    strophe_js.Strophe.addNamespace(
      "MUC_ROOMCONF",
      strophe_js.Strophe.NS.MUC + "#roomconfig"
    );
    return strophe_js.Strophe.addNamespace(
      "MUC_REGISTER",
      "jabber:iq:register"
    );
  },
  ...
  saveConfiguration: function(room, config, success_cb, error_cb) {
    ...
    if (
      typeof strophe_js.Strophe.x !== "undefined" &&
      typeof strophe_js.Strophe.x.Form !== "undefined" &&
      config instanceof strophe_js.Strophe.x.Form
    ) {
      config.type = "submit";
  ...
  test_append_nick: function(room, nick) {
    var domain, node;
    node = strophe_js.Strophe.escapeNode(
      strophe_js.Strophe.getNodeFromJid(room)
    );
...
XmppRoom = (function() {
  ...
  XmppRoom._parsePresence = function(pres) {
    ...
    data = {};
    data.nick = strophe_js.Strophe.getResourceFromJid(
      pres.getAttribute("from")
    );
  ...
  RoomConfig.prototype.parse = function(result) {
    ...
    for (i = 0, len = query.length; i < len; i++) {
      ...
      switch (child.nodeName) {
        ...
        case "x":
          if (
            !child.childNodes[0].getAttribute("var") === "FORM_TYPE" ||
